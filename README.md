...
- Giải X-O nội bộ 7 thành viên với vòng bảng, lucky member và playoff BO5.
- Gameplay, rating, ví điểm, pool bet và cược trực tiếp đều được xác nhận phía server và đồng bộ realtime.
- Phát hành theo hai giai đoạn test/live; xem [runbook X-O Challenger](docs/xo-challenger-release.md).
...
Chạy toàn bộ release gate X-O (unit, contract, pgTAP, concurrency và browser E2E) với Supabase local đang hoạt động:
```bash
