...
- At the compact container threshold, hide `.tab-label`, reduce horizontal tab padding, and give every visible tab an equal share of the navigation width.
...
2. **Compact:** All labels are hidden together when the navigation container crosses the compact threshold. Icons and unread badges remain visible and are distributed evenly across the available width.
