alter table public.citizen_wallets drop constraint if exists citizen_wallets_balance_check;
...
declare
  ...
  v_award integer := case when extract(isodow from (now() at time zone 'Asia/Ho_Chi_Minh')) in (6, 7) then 36 else 18 end;
  v_result_points integer := v_award;
...
begin
  ...
  if p_claim then
    ...
    values (p_member_id, v_award, 'daily_checkin', v_date)
    on conflict (member_id, grant_date) do nothing;
    get diagnostics v_inserted = row_count;

    if v_inserted > 0 then
      update public.citizen_wallets w
      set balance = w.balance + v_award, updated_at = now()
      where w.member_id = p_member_id;
    else
      v_result_points := -360;
      update public.citizen_wallets w
      set balance = w.balance - 360, updated_at = now()
      where w.member_id = p_member_id;
      insert into public.citizen_point_ledger(member_id, amount, reason)
      values (p_member_id, -360, 'daily_checkin_penalty');
    end if;
  end if;

  return query
  select w.balance, v_result_points, v_inserted > 0,
    exists (
